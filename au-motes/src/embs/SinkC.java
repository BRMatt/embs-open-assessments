...
public class SinkC {
	...
	private static int n = 7; // number of beacons of sync phase - sample only,
	...
	static {
		...
		rEnd.setCallback(new TimerEvent(null) {
			public void invoke(byte param, long time) {
				SinkC.stopReceivePeriod(param, time);
			}
		});
	...
	private static int onReceive(int flags, byte[] data, int len, int info,
		...
		if (inReceivePeriod) {
			Logger.appendString(csr.s2b("SinkC.: In Reception: "));
		} else {
			Logger.appendString(csr.s2b("SinkC.: NOT In Reception: "));
		}
